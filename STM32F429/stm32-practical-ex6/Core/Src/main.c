...
typedef struct
{
  ...
  // Измеренный ток
  float meas_current;
  // Измеренный ток (без фильтрации)
  float meas_current_unfiltered;
  // Измеренная угловая скорость
  float meas_velocity;
  // Измеренная угловая скорость (без фильтрации)
  float meas_velocity_unfiltered;
  // Измеренная угловая скорость в об\мин
...
int main(void)
{
  ...
  TIM7->ARR = 5 - 1; //PSC = 9000 - 1; f = 10KHz
